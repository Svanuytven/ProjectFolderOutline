...
```{r}
sapply(grep("^integrated_snn_res",colnames(bc.integrated@meta.data),value = TRUE),
...
```{r}
Idents(object = bc.integrated) <- "integrated_snn_res.0.6"
```

## Create table displaying the number of each sample belonging to each table

```{r}
table(bc.integrated@active.ident,bc.integrated@meta.data$method_timepoint)
```
